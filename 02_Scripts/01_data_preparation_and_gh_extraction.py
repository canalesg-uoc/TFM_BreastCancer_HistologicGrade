...
#
# Descripción:
# Consolida la descarga y el preprocesamiento inicial de los datos.
# 1. Descarga los datasets GSE4922 (principal) y GSE2990 (validación) de GEO.
# 2. Carga el SOFT de GSE4922, construye la matriz de expresión, mapea a genes,
#    resuelve probes duplicadas por máxima varianza y extrae la variable objetivo.
# 3. Guarda X (muestras x genes), Y (grado histológico) y el mapa de probes
#    en '01_Data/processed'.
# -----------------------------------------------------------------------------
"""
...
MAIN_GEO_ID = GEO_IDS[0] # GSE4922 es el dataset principal
RAW_DATA_PATH = os.path.join('01_Data', 'raw') 
PROCESSED_DATA_PATH = os.path.join('01_Data', 'processed')

# Archivos de salida para el dataset principal
OUTPUT_FILE_X = os.path.join(PROCESSED_DATA_PATH, '{}_expression_matrix.csv'.format(MAIN_GEO_ID))
OUTPUT_FILE_Y = os.path.join(PROCESSED_DATA_PATH, '{}_target_variable.csv'.format(MAIN_GEO_ID))
OUTPUT_FILE_MAP = os.path.join(PROCESSED_DATA_PATH, '{}_probe_to_gene_map.csv'.format(MAIN_GEO_ID))

# =============================================================================
# FUNCIONES AUXILIARES (del script 2 original)
# =============================================================================
...
# =============================================================================
# FASE 1: DESCARGA (del script 1 original)
# =============================================================================
...
# =============================================================================
# FASE 2: CARGA Y LIMPIEZA (del script 2 original)
# =============================================================================

def process_single_dataset(geo_id: str):
    """Carga, limpia y procesa un dataset GEO específico (ej. GSE4922)."""
    ...
    if not os.path.exists(gse_file_path):
        print(f"❌ Archivo no encontrado: {gse_file_path}", file=sys.stderr)
        print("   Asegúrate de haber ejecutado la descarga previamente o verifica la ruta.", file=sys.stderr)
        return
    ...
    if not X.index.isin(probe_to_gene_map.index).any():
        print("⚠️ Aviso: El índice de X no coincide con IDs de sonda de la GPL.", file=sys.stderr)
    ...
    # 5) Extracción y Limpieza de Variable Objetivo Y
    ...
    # Guardar resultados
    y.index.name = "Sample"
    X_final.T.to_csv(OUTPUT_FILE_X) # Se guarda como (muestras x genes)
    y.to_csv(OUTPUT_FILE_Y)

    print("\n✅ Datos preprocesados guardados exitosamente.")
    print(f"   X (muestras x genes): {X_final.T.shape} -> {OUTPUT_FILE_X}")
    print(f"   Y (muestras): {y.shape} -> {OUTPUT_FILE_Y}")
...
def main():
    try:
        ...
        # FASE 2: Preprocesamiento del dataset principal (GSE4922)
